#!/usr/bin/env python3
"""Map Noida workspace CSV → listing JSON for seed-noida-csv.ts."""
...
CSV_PATH = Path(__file__).resolve().parents[4] / "noida-workspaces-2026-07-23.csv"
OUT_PATH = Path(__file__).resolve().parent / "noida-csv-listings.json"
...
def operator_from_row(name: str, brand: str) -> str:
    ...
    if not brand_clean or brand_clean.lower() in {"other coworking", "other"}:
        return clean(name)
    # Title-case common brands while preserving known casing like AltF, 91Springboard
    if brand_clean.lower() == "91springboard":
        return "91Springboard"
    if brand_clean.lower() == "altf":
        return "AltF"
