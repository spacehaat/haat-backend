#!/usr/bin/env python3
"""Map data (14).csv Mumbai website leads to seed JSON (deduped, seats as-is)."""
...
CSV_PATH = ROOT / 'data (14).csv'
OUT_PATH = Path(__file__).resolve().parent / 'mumbai-csv-leads.json'
...
INTERESTED = {
    ...
    'virtual office': 'Hot desk',
    'enterprise solution': 'Managed office',
}
...
def norm_city(value: str) -> str:
    ...
    return value.capitalize() if value else 'Mumbai'
...
def main() -> None:
    ...
    for row_num, row in enumerate(rows, start=2):
        ...
        seat_range = norm_text(row.get('Number of Seats', ''))
    ...
    for item in unique:
        groups[item['softKey']].append(item)
